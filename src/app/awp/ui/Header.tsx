...
export default function Header() {
  ...
  return (
    <div className="flex justify-between items-center h-[115px] px-2 w-[95%] bg-white mx-auto my-4 rounded-3xl">
      ...
      <nav>
        <ul className="flex gap-5 flex-1 justify-center items-center">
          {links.map((link, index) => (
            <li key={index} className={clsx(' min-w-5 py-2 px-4 rounded-full', {"bg-[#7CB41C]": pathname === link.href, "hover:bg-[#F0F2F4]": pathname !== link.href })}>
              <Link
                key={link.name}
                href={link.href}
      ...
      <div className="min-w-[165px] mx-6">
        <button className=" ">
          ...
          >
            <span className="rounded-full px-6 py-3 bg-white text-black font-medium transition-colors duration-300
